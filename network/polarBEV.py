...
class polarBEV(nn.Module):
    ...
    def __init__(self, grid_size, pt_model = 'pointnet', fea_dim = 3, pt_pooling = 'max', kernal_size = 3,
                 out_pt_fea_dim = 64, max_pt_per_encode = 64, cluster_num = 4, pt_selection = 'farthest', fea_compre = None):
        super(polarBEV, self).__init__()
        ...
        # Polar processing
        self.Unet_encoder = UNet_encoder(n_height=64, dilation=1, group_conv=False, input_batch_norm=True, dropout=0.5, circular_padding=True, dropblock=True)
    ...
    def forward(self, pt_fea, xy_ind, voxel_fea=None):
        ...
        x1_polar, x2_polar, x3_polar = self.Unet_encoder(out_data)
        polar_data = [x1_polar, x2_polar, x3_polar]
        return polar_data
