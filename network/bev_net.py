...
class BEVFusionv2(nn.Module):
    def __init__(self, channel):
        ...
        self.attention_polar = nn.Sequential(
             nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(channel, channel, kernel_size=1),
            nn.Sigmoid()
        )
    ...
    def forward(self, bev_features, sem_features, com_features, polar_feature):
        sem_features = self.adapter_sem(sem_features)
        com_features = self.adapter_com(com_features)
        attn_bev = self.attention_bev(bev_features)
        attn_sem = self.attention_sem(sem_features)
        attn_com = self.attention_com(com_features)
        attn_polar = self.attention_polar(polar_feature)

        fusion_features = torch.mul(bev_features, attn_bev) \
            + torch.mul(sem_features, attn_sem) \
            + torch.mul(com_features, attn_com) + torch.mul(polar_feature, attn_polar)
...
class BEVUNetv1(nn.Module):
    def __init__(self, n_class, n_height, dilation, bilinear, group_conv, input_batch_norm, dropout, circular_padding, dropblock):
        ...
        # instance decoder
        self.i_up4_center = up(192, 64, circular_padding, bilinear=bilinear, group_conv=group_conv, use_dropblock=dropblock, drop_p=dropout)
        self.i_up4_offset = up(192, 64, circular_padding, bilinear=bilinear, group_conv=group_conv, use_dropblock=dropblock, drop_p=dropout)
        
        # instance head
        self.i_outc_center = outconv(64, 1)
        self.i_outc_offset = outconv(64, 2)
...
class BEVUNetvCFFE(nn.Module):
    def __init__(self, n_class, n_height, dilation, bilinear, group_conv, input_batch_norm, dropout, circular_padding, dropblock):
        ...
        # instance head
        self.i_outc_center = nn.Sequential(
            outconv(64, 1),
            nn.Sigmoid()
        )
        # outconv(64, 1)
    ...
    def forward(self, x, sem_fea_list, com_fea_list):
        ...
        x = self.up3(x, x2_f)  # 256, 128
        s_x = self.up4(x, x1)  # 128, 64
        sem_x = self.outc(self.dropout(s_x))
        ...
        # return s_x, sem_x, i_x_center, i_x_offset
        return sem_x, i_x_center, i_x_offset

class BEVUNetv3(nn.Module):
    def __init__(self, n_class, n_height, dilation, bilinear, group_conv, input_batch_norm, dropout, circular_padding, dropblock):
        super().__init__()
        self.inc = inconv(64, 64, dilation, input_batch_norm, circular_padding)
        self.down1 = down(64, 128, dilation, group_conv, circular_padding)
        ...
        channels = [128, 256, 512]
        self.bev_fusions = nn.ModuleList([BEVFusionv2(channels[i]) for i in range(3)])

    def forward(self, x, sem_fea_list, com_fea_list, polar_feat_list):
        x1 = self.inc(x)    # [B, 64, 256, 256]
        x2 = self.down1(x1)    # [B, 128, 128, 128]
        x2_f = self.bev_fusions[0](x2, sem_fea_list[0], com_fea_list[0], polar_feat_list[0]) # 128, 64, 64 -> 128
        x3 = self.down2(x2_f)    # [B, 256, 64, 64]
        x3_f = self.bev_fusions[1](x3, sem_fea_list[1], com_fea_list[1], polar_feat_list[1]) # 256, 128, 128 -> 256
        x4 = self.down3(x3_f)    # [B, 512, 32, 32]
        x4_f = self.bev_fusions[2](x4, sem_fea_list[2], com_fea_list[2], polar_feat_list[2]) # 512, 256, 256 -> 512
        x5 = self.down4(x4_f)    # [B, 512, 16, 16]
        x = self.up1(x5, x4_f)  # 512, 512
        x = self.up2(x, x3_f)  # 512, 256
        x = self.up3(x, x2_f)  # 256, 128
