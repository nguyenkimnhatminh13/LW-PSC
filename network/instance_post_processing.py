...
def find_instance_center(ctr_hmp, threshold=0.1, nms_kernel=5, top_k=None, polar=False):
    ...
    if polar:
        ...
        ctr_hmp_max_pooled = F.pad(
            ctr_hmp, (nms_padding, nms_padding, 0, 0), mode="circular"
        )
        ctr_hmp_max_pooled = F.max_pool2d(
            ctr_hmp_max_pooled,
            kernel_size=nms_kernel,
            stride=1,
            padding=(nms_padding, 0),
        )
    else:
        nms_padding = (nms_kernel - 1) // 2
        ctr_hmp_max_pooled = F.max_pool2d(
...
def group_pixels(ctr, offsets, polar=False):
    ...
    x_coord = torch.arange(width, dtype=offsets.dtype, device=offsets.device).repeat(
        1, height, 1
    )
    ...
    if polar:
        distance[:, :, 0] = torch.add(
            torch.fmod(torch.add(distance[:, :, 0], width / 2), width), -width / 2
        )
...
def get_instance_segmentation(
    ...
    #     ctr_hmp, threshold=threshold, nms_kernel=nms_kernel, top_k=top_k, polar=polar
    # )
    # conf = np.ones([1,1,256,256], dtype=np.float32)
    # conf_t = torch.from_numpy(conf).to(ctr_hmp.device)
    pv_nms = pytorch_lib.PointVoteNMS(dict(K=100, score_thresh=0.2, dist_thresh=0.8, vote_thresh=0.4))
    ctr = pv_nms.forward(
        sem_seg,
        offsets,
        ctr_hmp,
...
def merge_semantic_and_instance(
    sem_seg, sem, ins_seg, label_divisor, thing_list, void_label, thing_seg
):
    """
    ...
    semantic_thing_seg = (sem_seg > 0) & (sem_seg <= max(thing_list))
    ...
    if not torch.nonzero(thing_mask).size(0) == 0:
        sem_sum = torch_scatter.scatter_add(
            sem.permute(0, 2, 3, 4, 1)[thing_mask], ins_seg[thing_mask], dim=0
        )
        class_id = torch.argmax(sem_sum[:, 0: max(thing_list) + 1], dim=1)
        sem_seg[thing_mask] = (
            (ins_seg[thing_mask] * label_divisor) + class_id[ins_seg[thing_mask]]
        )
    else:
...
def get_panoptic_segmentation(
    ...
    if sem.dim() != 5 and sem.dim() != 4:
        raise ValueError(
            "Semantic prediction with un-supported dimension: {}.".format(sem.dim())
        )
    ...
    if foreground_mask is not None:
        if foreground_mask.dim() != 4:
            raise ValueError(
                "Foreground prediction with un-supported dimension: {}.".format(
                    sem.dim()
                )
            )
    ...
    if sem.dim() == 5:
        ...
        semantic = sem.type(torch.LongTensor).cuda()
        # shift back to original label idx
        # semantic = torch.add(semantic, 1).type(torch.LongTensor).cuda()
        one_hot = torch.zeros(
            (
                sem.size(0),
                # torch.max(semantic).item() + 1,
                20,
                sem.size(1),
                sem.size(2),
                sem.size(3),
        ...
        sem = one_hot.scatter_(1, torch.unsqueeze(semantic.long(), 1), 1.0)
        # sem = sem[:, 1:, :, :, :]
    ...
    panoptic = merge_semantic_and_instance(
        semantic, sem, instance, label_divisor, thing_list, void_label, thing_seg
    )

    return panoptic, center


# def get_panoptic_segmentation_cfnet(
#     sem,
#     ctr_hmp,
#     offsets,
#     thing_list,
...
#     instance, center = get_instance_segmentation_cfnet(
#         semantic,
#         ctr_hmp,
#         offsets,
#         thing_list,
...
# def get_instance_segmentation_cfnet(
#     sem_seg,
#     ctr_hmp,
#     offsets,
#     thing_list,
...
#     sem_seg 
#     k = 100
#     obj_center = torch.zeros((k, 3), dtype=torch.float32, device=sem_seg.device)
#     grid_score = torch.cat(,)
    
#     valid_fg_mask = (sem_seg[:,1:3] >= 1) * (sem_seg[:,1:3] <= 8)
#     valid_high_score_mask = 
