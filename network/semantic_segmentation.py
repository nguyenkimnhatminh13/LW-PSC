...
class SemanticBranch(nn.Module):
    ...
    def forward(self, data_dict, labels):
        if self.phase == 'trainval':
            out_dict = self.forward_once(data_dict['vw_features'], 
                data_dict['coord_ind'], data_dict['full_coord'], labels, data_dict['info'])
            ...
            for i in range(len(all_teach_pair)):
                ...
                # loss_dict["vw_" + str(i) + "lovasz_loss"] = 0
                # loss_dict["vw_" + str(i) + "ce_loss"] = 0
            return dict(
