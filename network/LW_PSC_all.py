...
from .bev_net import outconv, BEVUNetv3, BEVUNetvCF
...
class LW_PSC_all(nn.Module):
    def __init__(
        ...
        self.bev_model = BEVUNetv3(self.nbr_classes*self.n_height, self.n_height, self.dilation, self.bilinear, self.group_conv,
                            self.input_batch_norm, self.dropout, self.circular_padding, self.dropblock)
        # self.bev_model_v2 = BEVUNetv2(self.nbr_classes*self.n_height, self.n_height, self.dilation, self.bilinear, self.group_conv,
        #                     self.input_batch_norm, self.dropout, self.circular_padding, self.dropblock)
        ...
        self.dropout = nn.Dropout(p=0. if self.dropblock else self.dropout)
    ...
    def forward(self, occupancy, points, labels, aux_com, distance_feature, grid_polar_ind, polar_fea):
        ...
        s_x, x_pred, center_pred, offset_pred = self.bev_model(inputs, ss_out_dict['mss_bev_dense'], sc_out_dict['mss_bev_dense'], polar_feat_list)
        ...
        for b in range(batch_size):
            ...
            # sem_diff0_ind_BEV = torch.stack((x.float(), y.float()), axis=-1).unsqueeze(dim=0).unsqueeze(dim=3)
            # sem_diff0_ind_BEV_batch.append(sem_diff0_ind_BEV)
            ...
            # x_thing, y_thing, z_thing = torch.where((semantic[b] >= 1) & (semantic[b] <= 8))
            # # thing_ind_from_panoptic = np.stack((x_thing, y_thing, z_thing), axis=-1)
            # x_cluster = x_thing + offset_pred[b, 0, x_thing, y_thing]
            # y_cluster = y_thing + offset_pred[b, 1, x_thing, y_thing]
            ...
            # )
            # ax1.set_title("Shift point Results")
            ...
            # )
            # ax2.set_title("Original point results")
...
class LW_PSC_all_CF(nn.Module):
    def __init__(
        ...
        self.sem_branch = SemanticBranch(sizes=self.input_dimensions, nbr_class=self.nbr_classes-1, init_size=self.n_height, class_frequencies=ss_weight, phase=phase)
        self.com_branch = CompletionBranch(init_size=self.n_height, nbr_class=self.nbr_classes, phase=phase)
        self.bev_model = BEVUNetvCF(self.nbr_classes*self.n_height, self.n_height, self.dilation, self.bilinear, self.group_conv,
                            self.input_batch_norm, self.dropout, self.circular_padding, self.dropblock)
        # self.bev_model_v2 = BEVUNetv2(self.nbr_classes*self.n_height, self.n_height, self.dilation, self.bilinear, self.group_conv,
        #                     self.input_batch_norm, self.dropout, self.circular_padding, self.dropblock)
        self.voxel2bev = Voxel2BEV_module(output_size=(self.input_dimensions[0], self.input_dimensions[1]), scale_rate = (1, 1))
        ...
        self.i_outc_offset = outconv(128, 2)
        # self.i_outc_heatmap = nn.Sequential(
        #     outconv(128, 1),
        #     nn.Sigmoid()
        # )
        self.i_outc_heatmap = outconv(128, 1)
    ...
    def forward(self, occupancy, points, labels, aux_com, distance_feature, grid_polar_ind, polar_fea):
        ...
        for b in range(batch_size):
            ...
            # sem_diff0_ind_BEV_batch.append(sem_diff0_ind_BEV)
            sem_diff0_feat = x_pred[b,:, x, y, z].unsqueeze(dim=0).unsqueeze(dim=3)
            sem_diff0_center_sig = center_pred[b,:, x, y].unsqueeze(dim=0).unsqueeze(dim=3)
            sem_diff0_offset = offset_pred[b,:, x, y].unsqueeze(dim=0).unsqueeze(dim=3)
        ...
        return preds_list, aux_ss_loss, aux_sc_loss