...
class SemKITTI(data.Dataset):
    def __init__(
        ...
        self.im_idx_small = [file for idx, file in enumerate(self.im_idx) if idx % 5 == 0]
        self.occ_idx_small = [file for idx, file in enumerate(self.occ_idx) if idx % 5 == 0]
        self.label_1_2_idx_small = [file for idx, file in enumerate(self.label_1_2_idx) if idx % 5 == 0]
        self.invalid_1_2_idx_small = [file for idx, file in enumerate(self.invalid_1_2_idx) if idx % 5 == 0]
        self.label_1_4_idx_small = [file for idx, file in enumerate(self.label_1_4_idx) if idx % 5 == 0]
        self.invalid_1_4_idx_small = [file for idx, file in enumerate(self.invalid_1_4_idx) if idx % 5 == 0]
        self.label_1_8_idx_small = [file for idx, file in enumerate(self.label_1_8_idx) if idx % 5 == 0]
        self.invalid_1_8_idx_small = [file for idx, file in enumerate(self.invalid_1_8_idx) if idx % 5 == 0]
    ...
    def __len__(self):
        ...
        return len(self.im_idx_small)

    def __getitem__(self, index):
        raw_data = np.fromfile(self.im_idx_small[index], dtype=np.float32).reshape((-1, 4))
        ...
        if self.imageset == "test":
            ...
            # ).reshape((-1, 1))
            annotated_data = np.fromfile(
                self.im_idx_small[index].replace("velodyne", "labels")[:-3] + "label",
                dtype=np.uint32,
            ).reshape(-1)
        ...
        data_sem = psc_data["semantic_labels"].astype(np.uint8)
        data_ins = psc_data["instance_labels"].astype(np.uint8)


        data_tuple += (data_sem, data_ins)
...
class voxel_dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        (
            points, labels, _, dense_label_sem, dense_label_ins, occupancy, label_1_2, invalid_1_2, label_1_4, invalid_1_4, label_1_8, invalid_1_8
        ) = data
        ...
        ins_diff0_ind = np.stack((x_ins, y_ins, z_ins), axis=-1)
        if self.flip_aug:
        ...
        if inst_label.size > 1:
            ...
            inst_voxel_pair = np.concatenate(
                [ins_diff0_ind[:,:2], inst_label[..., np.newaxis]],
                axis=1,
            )
            inst_voxel_pair = inst_voxel_pair[
                np.lexsort((ins_diff0_ind[:,0], ins_diff0_ind[:,1])),
                :,
            ]
        ...
        xyz_inst = ins_diff0_ind * intervals_voxel + min_bound
