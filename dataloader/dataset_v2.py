...
class SemKITTI_v2(data.Dataset):
    def __init__(
...
def collate_fn_BEV(data):
    ...
    # processed_inst2stack = np.stack([d[4] for d in data])
    ...
    invalid_1_2 = np.stack([d[9] for d in data])
    label_1_4 = np.stack([d[10] for d in data])
    invalid_1_4 = np.stack([d[11] for d in data])
    label_1_8 = np.stack([d[12] for d in data])
    invalid_1_8 = np.stack([d[13] for d in data])
    aux_com = {
        "label_1_2": torch.from_numpy(label_1_2),
        "invalid_1_2": torch.from_numpy(invalid_1_2),
        "label_1_4": torch.from_numpy(label_1_4),
    ...
    return (
        ...
        # torch.from_numpy(processed_inst2stack),
...
def collate_fn_BEV_addPolar(data):
    ...
    labels = [d[2] for d in data]
    ssclabel2stack = np.stack([d[3] for d in data])
    BEV_instance2stack = np.stack([d[4] for d in data])
    center2stack = np.stack([d[5] for d in data])
    offset2stack = np.stack([d[6] for d in data])
    flatten_label = [d[7] for d in data]
    flatten_inst = [d[8] for d in data]
    sem_diff255_ind = [d[9] for d in data]
    label_1_2 = np.stack([d[10] for d in data])
    invalid_1_2 = np.stack([d[11] for d in data])
    label_1_4 = np.stack([d[12] for d in data])
    invalid_1_4 = np.stack([d[13] for d in data])
    label_1_8 = np.stack([d[14] for d in data])
    invalid_1_8 = np.stack([d[15] for d in data])
    aux_com = {
        "label_1_2": torch.from_numpy(label_1_2),
        "invalid_1_2": torch.from_numpy(invalid_1_2),
        "label_1_4": torch.from_numpy(label_1_4),
    ...
    distance_feature = np.stack([d[16] for d in data]).astype(np.float32)
    grid_polar_ind = [d[17] for d in data]
    return_polar_feat = [d[18] for d in data]
    ...
    return (
        ...
        labels,
        torch.from_numpy(ssclabel2stack),
        torch.from_numpy(BEV_instance2stack),
