...
def _read_label2_SemKITTI(path):
    point_label = _read_SemKITTI(path, dtype=np.uint32, do_unpack=False).reshape(
        (-1, 1)
    )
    point_label = point_label & 0xFFFF
    return point_label
...
def _read_point_SemKITTI(path):
    point = _read_SemKITTI(path, dtype=np.float32, do_unpack=False).reshape(-1, 4)
    return point
