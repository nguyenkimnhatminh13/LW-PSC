...
class PointVoteNMS(nn.Module):
    ...
    def forward(self, pred_sem, pred_offset, pred_hmap):
        '''
        Input:
            # pcds_xyz, (N, C), C -> (x, y, z, intensity, ...)
            pred_sem, (N, class_num, H, W, Z)
            pred_offset, (N, 2, H, W)
            pred_hmap, (N, 1, H, W)
        Output:
            obj_center, (1, K, 2)
        ...
        # pred_semantic = pred_sem.argmax(dim=1)
        pred_semantic = pred_sem

        voxel_position = torch.from_numpy(np.indices([256,256])).unsqueeze(0)
        voxel_position = voxel_position.to(pred_sem.device).float()  # (1, 2, H, W)
        
        obj_center = torch.zeros((self.K, 3), dtype=voxel_position.dtype, device=pred_sem.device)
        pcds_xy_score = torch.cat((voxel_position + pred_offset, pred_hmap), dim=1) # (N, 3, H, W)
        pcds_xy_score = pcds_xy_score.permute(0, 2, 3, 1).contiguous().reshape(-1,3) # (N, H, W, 3) -> (N, 3)

        # get foreground and high-confidence mask
        # gets foreground segmentation
        thing_seg = torch.zeros_like(pred_semantic)
        for thing_class in [1,2,3,4,5,6,7,8]:
            thing_seg[pred_semantic == thing_class] = 1
        # [1, H, W, Z] --> [1, H, W]
        thing_seg = torch.max(thing_seg,dim=3)
        valid_fg_mask = (thing_seg.values > 0).view(-1) # (N, H, W)
        
        # valid_fg_mask = (pred_semantic >= 1) * (pred_semantic <= 8)
        valid_high_score_mask = (pred_hmap > self.score_thresh).view(-1) * valid_fg_mask # (N, H, W)
        pcds_fg = pcds_xy_score[valid_high_score_mask]
        ...
        if N_fg > 1:
            _, indices = pcds_fg[:, -1].sort(dim=0, descending=True)
            pcds_fg_order = pcds_fg[indices]
            # pcds_fg_order_unique = torch.unique(pcds_fg_order, dim=0)
            
            matching_mat = torch.zeros((N_fg, col_blocks), dtype=torch.long, device=pred_sem.device)
            matching_mat_vote = torch.zeros((N_fg, col_blocks), dtype=torch.long, device=pred_sem.device)
            remv = torch.zeros((col_blocks,), dtype=torch.long, device=pred_sem.device)
            keep = torch.zeros((self.K,), dtype=torch.long, device=pred_sem.device)

            pan_lib.cuda_kernel.vote_nms_fast_gpu(pcds_fg_order, obj_center, matching_mat, matching_mat_vote, remv, keep, self.dist_thresh, self.vote_thresh)
            valid_center_mask = obj_center.abs().sum(dim=1) > 0
            obj_center = obj_center[valid_center_mask][:,:2].int()  # (K, 2)
        else:
            obj_center = torch.empty((0,2), dtype=torch.int64, device=pred_sem.device)
            # # assign instance label
            # dist = (obj_center_valid.unsqueeze(0) - pcds_xy_score[valid_fg_mask][:, :2].unsqueeze(1)).norm(dim=-1) #(N1, K1)
            # instance_id = dist.argmin(dim=1) + 1

        #     # # majarity voting
        #     # sem_ins_index = torch.stack((pred_semantic[valid_fg_mask], instance_id), dim=1).view(1, -1, 2, 1) #(1, N1, 2, 1)
        #     # pred_sem_ins_vote = VoxelSum(pcds_ind=sem_ins_index, output_size=(100, int(instance_id.max()) + 1)) #(1, class_num, gt_obj_max)
        #     # _, pred_sem_ins_vote_id = pred_sem_ins_vote.max(dim=1, keepdim=True) #(1, 1, gt_obj_max)
        #     # pred_sem_vote = VoxelQuery(voxel_in=pred_sem_ins_vote_id, pcds_ind=instance_id.view(1, -1, 1, 1)).view(-1) #(N1)
        ...
        #     # assert(int(pred_sem_vote.min()) >= 1)
        #     # add instance_id
        #     pred_semantic[valid_fg_mask] = pred_sem_vote
        #     pred_semantic[valid_fg_mask] += instance_id << 16
            # return obj_center_valid, pred_semantic
        # else:
        #     return None, pred_semantic
